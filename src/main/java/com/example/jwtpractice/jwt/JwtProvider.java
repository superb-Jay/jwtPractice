...
public class JwtProvider {
    ...
    //토큰을 발급하거나, 토큰을 claims로 바꾸는 클래스 이다.

    private final JwtProperties jwtProperties;

    public String makeJwtToken(Member member) { //토큰 발급 시기를 생각해보면 토큰은 로그인하고나서 (findByEmailAndPassword) 실행후이므로 실행 결과값은 Optional<entity> 나옴
        ...
        return Jwts.builder()
                ...
                .claim("email", member.getEmail()) // 페이로드에 현재 엔티티의 정보
                .signWith(SignatureAlgorithm.HS256, jwtProperties.getSecretKey())
                .compact();
    }

    public RequestMemberDTO tokenToRequestMember(String token) {
        ...
        if (validationToken(token)) {//토큰이 Bearer로 시작하는지 형식이 맞는지 확인
            ...
            return new RequestMemberDTO(claims);
        }
        return null;
    }
